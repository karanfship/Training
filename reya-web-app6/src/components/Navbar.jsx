    ...
    return (
        <div className='flex justify-between items-center h-24 max-w-[1240px] mx-auto px-4 text-white'>
            ...
            <div className={nav ? 'fixed left-0 top-0 w-[60%] h-full border-r border-r-gray-900 bg-[#000300] ease-in-out duration-500' : 'ease-in-out duration-500 fixed left-[-100%]'}>
                ...
                <div className='flex text-lg text-white hover:text-[#00df9a] hover:cursor-pointer'>
                    <Link to="/checkout">
                        <div className="flex text-lg text-white hover:text-[#00df9a] hover:cursor-pointer">
                            <GiShoppingCart className='text-white text-3xl ' />
                            <span className="">
            ...
            <Link to={!user && "/login"}>
                <button onClick={handleAuthentication} className='bg-[#00df9a] w-[200px] rounded-md whitespace-nowrap font-bold ml-6 mx-auto p-2 text-gray-900'>{user ? 'Sign Out' : 'Sign In'}</button>
            </Link>
        </div>
    )
}
